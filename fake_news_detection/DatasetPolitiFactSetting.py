...
class DatasetPolitiFactSetting(dataset):
    data = None
    ...
    def load(self):
        ...
        if 'large' in self.dataset_source_folder_path:
            f = open('../../result/expFake_News/sample_result_large_10_', 'rb')
        else:
            f = open('../../result/expFake_News/sample_result_2_', 'rb')
        setting_dict = pickle.load(f)
        f.close()
        
        return {'node': node_dict, 'link': link_dict, 'setting': setting_dict}
